...
async Task SaveCachePg(string qnorm, string answer, int ttlMinutes)
{
    ...
    await using var cmd = new NpgsqlCommand(@"
        ...
        VALUES (@q, @a, NOW() + (@ttl || ' minutes')::interval);
    ", conn);

    cmd.Parameters.AddWithValue("q", qnorm);
    cmd.Parameters.AddWithValue("a", answer);
    cmd.Parameters.AddWithValue("ttl", ttlMinutes.ToString());
...
async Task<string> AskOpenAI(string question, bool useWeb)
{
    ...
    if (string.IsNullOrWhiteSpace(key))
        throw new Exception("OPENAI_KEY yok.");
    ...
    var body = new Dictionary<string, object?>
    {
        ...
        ["input"] = new object[]
        {
            ...
            {
                ["role"] = "system",
                ["content"] = "Sen VR öğretmensin. Her zaman Türkçe, kısa ve net cevap ver."
            },
            new Dictionary<string, string>
            {
                ["role"] = "user",
...
app.MapGet("/dbcheck", async () =>
{
    try
    {
        ...
        return Results.Ok(new { ok = true, dbTime = result?.ToString() });
    }
    catch (Exception ex)
    {
...
app.MapPost("/ask", async (HttpRequest request) =>
{
    ...
    var answer = await AskOpenAI(question, timeSensitive);

    var ttl = timeSensitive ? 10 : (60 * 24 * 30);

    await SaveCachePg(qnorm, answer, ttl);
